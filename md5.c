 ...
 */
void
MD5_Update(MD5_CTX *ctx, const u_int8_t *input, size_t len)
{
	size_t have, need;
...
/*	ctx->count += (uint64_t)len << 3;*/
	if ((ctx->count[0] += ((u_int32_t)len << 3)) < (u_int32_t)len) {
	/* Overflowed ctx->count[0] */
		ctx->count[1]++;
	}
	ctx->count[1] += ((u_int32_t)len >> 29);
 ...
 */
void
MD5_Final(u_int8_t digest[MD5_DIGEST_LENGTH], MD5_CTX *ctx)
{
	u_int8_t count[8];
	size_t padlen;
	int i;
 ...
 */
void
MD5_Transform(u_int32_t state[4], const u_int8_t block[MD5_BLOCK_LENGTH])
{
